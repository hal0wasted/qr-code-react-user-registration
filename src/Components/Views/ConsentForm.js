  ...
  render(){
    ...
    return (
      <div>
        ...
        <h1 className='consent-form-header'>Consent Form</h1>
        <div className='consent-form-top-spacer'></div>
        ...
        <form onSubmit={ this.props.handleSubmit }>
          <div className='consent-form-div'>Name: {userValues.firstName} {userValues.lastName}</div>
          <div className='consent-form-div'>Date: {today}</div>
          <div className='consent-form-div'>Digital Signature:&nbsp;&nbsp;&nbsp;</div>
          <div className='consent-form-div'>
...
class ConsentFormMain extends Component {
  submit = async (values) => {
    const { userValues, hideConsentForm, showDemoSurvey } = this.props
    if (values.digitalSignature === `${userValues.firstName} ${userValues.lastName}`){
      try {
        const response = await axios.post(`${protocol}://${host.getCurrent()}:${port}/consentFormSubmit`, { data: values.digitalSignature })
  ...
  render(){
    return (
      <div>
        <ConsentForm onSubmit={ this.submit }/>
      </div>
    )
  }
}

export default connect(mapState, mapDispatch)(ConsentFormMain)
